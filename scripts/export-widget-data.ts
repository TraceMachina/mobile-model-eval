...
function getToolNameShort(name: string): string {
  return stripToolNamespace(name);
}
...
function processClaudeRawOutput(
  ...
  for (const [i, line] of lines.entries()) {
    ...
    if (obj.type === "assistant" && obj.message?.content) {
      ...
      if (usage) {
        ...
        const previousOutputTokens: number = pendingUsage?.outputTokens ?? 0;
        pendingUsage = {
          inputTokens: inp,
          outputTokens: Math.max(previousOutputTokens, out),
          thinkingTokens: 0,
        };
      }
    }
  ...
  if (pendingMessages.length > 0 && events.length > 0) {
    ...
    if (lastEvent) {
      const lastOffset = lastEvent.videoOffsetMs;
      for (const msg of pendingMessages) {
        events.push({
          type: "message",
  ...
  if (events.length > 0) {
    ...
    if (lastEvent) {
      events.push({
        type: "usage",
        videoOffsetMs: lastEvent.videoOffsetMs,
        inputTokens: cumulativeInputTokens,
        outputTokens: cumulativeOutputTokens,
        thinkingTokens: cumulativeThinkingTokens,
      });
...
function processGeminiRawOutput(
  ...
  for (const line of lines) {
    ...
    if (obj.type === "tool_result") {
      ...
      if (!toolUse) continue;
      const shortName = toolUse.name ? stripToolNamespace(toolUse.name) : "";
  ...
  if (pendingText.trim() && events.length > 0) {
    const lastEvent = events.at(-1);
    if (lastEvent) {
      const msgEvent: WidgetEvent = { type: "message", videoOffsetMs: lastEvent.videoOffsetMs };
      if (isThinkingBlock) {
        msgEvent.thinking = pendingText.trim();
      } else {
        msgEvent.text = pendingText.trim();
  ...
  for (const line of lines) {
    ...
    try {
      ...
      for (const tc of (bus.toolCalls || [])) {
        ...
        if (!callId) continue;
        const callName = tc.request?.name ? stripToolNamespace(tc.request.name) : "";
