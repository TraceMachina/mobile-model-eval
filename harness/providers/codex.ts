...
export interface CodexProviderOptions {
  ...
  mcpServerUrl: string;
  /** MCP server name to register with codex. */
...
export class CodexProvider implements LlmProvider {
  ...
  async setup(): Promise<void> {
    ...
    await this.spawn(["mcp", "remove", this.mcpServerName]).catch(() => {});
    const { exitCode, stderr } = await this.spawn([
      "mcp", "add", "--url", this.mcpServerUrl, this.mcpServerName,
    ]);
    if (exitCode !== 0) {
  ...
  async execute(prompt: string, config: ProviderConfig): Promise<LlmExecutionResult> {
    ...
    const deviceLabel = config.platform === "ios" ? "iOS simulator" : "Android device";

    const fullPrompt = [
      `You are controlling a ${deviceLabel} via MCP tools.`,
